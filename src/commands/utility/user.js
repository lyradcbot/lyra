...
module.exports = {
	...
	async execute(interaction) {
		...
		async function badge(flags) {
			...
			flags = flags.join('');
			flags = flags.replace(/Staff/g, '<:staff_dc:927229606124191754>').replace(/Hypesquad/g, '<:hypersquad:927229968176525382>').replace(/HypeSquadOnlineHouse2/g, '<:hps_brillinance:927230429805834260>').replace(/HypeSquadOnlineHouse1/g, '<:bravery:927236752643600404>').replace(/HypeSquadOnlineHouse3/g, '<:balance:927236471017058365>').replace(/PremiumEarlySupporter/g, '<:porcokkkkk:927230734391967754>').replace(/BugHunterLevel2/g, '<:bug_lvl2:927230992593334292>').replace(/VerifiedDeveloper/g, '<:bot_developer:927231423063150622>').replace(/CertifiedModerator/g, '<:moderator:927232655836184626>').replace(/Partner/g, '<:partner:927243125741719573>').replace(/BugHunterLevel1/g, '<:bug:927244179128602625>').replace(/VerifiedBot/g, '<:botv:927248783576817674>').replace(/BOT/g, '<:bot:927249890503950336>');
			return flags;
		}
