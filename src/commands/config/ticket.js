...
module.exports = {
	...
	async execute (interaction) {
		if (interaction.options.getSubcommand() === 'canal') {
			...
			return interaction.reply({ content: `O canal de tickets foi definido para <#${channel.id}>.`, ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'categoria') {
			const category = interaction.options.getChannel('categoria');
			if (category.type !== ChannelType.GuildCategory) {
				return interaction.reply({ content: 'A categoria precisa ser uma categoria.', ephemeral: true });
			}
			await interaction.client.db.guild.setTicketsCategory(interaction.guild.id, category.id);
			return interaction.reply({ content: `A categoria de tickets foi definida para <#${category.id}>.`, ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'status') {
			const type = interaction.options.getString('tipo');
			if (type === 'on') {
				await interaction.client.db.guild.setTickets(interaction.guild.id, true);
				return interaction.reply({ content: 'O sistema de tickets foi ativado.', ephemeral: true });
			}
			if (type === 'off') {
				await interaction.client.db.guild.setTickets(interaction.guild.id, false);
				return interaction.reply({ content: 'O sistema de tickets foi desativado.', ephemeral: true });
			}
			return interaction.reply({ content: 'Você precisa escolher entre `on` e `off`.', ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'logs') {
			...
			return interaction.reply({ content: `O canal de logs foi definido para <#${channel.id}>.`, ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'autotranscript') {
			...
			if (type === 'on') {
				...
				return interaction.reply({ content: 'O sistema de transcript automático foi ativado.', ephemeral: true });
			}
			if (type === 'off') {
				await interaction.client.db.guild.setTicketsTranscript(interaction.guild.id, false);
				return interaction.reply({ content: 'O sistema de transcript automático foi desativado.', ephemeral: true });
			}
			return interaction.reply({ content: 'Você precisa escolher entre `on` e `off`.', ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'transcript') {
			...
			return interaction.reply({ content: `O canal de transcript foi definido para <#${channel.id}>.`, ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'embed') {
			...
			return interaction.reply({ content: 'A embed foi definida.', ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'enviar') {
			...
			if (!channel) {
				return interaction.reply({ content: 'O canal de tickets não foi definido.', ephemeral: true });
			}
			if (!category) {
				return interaction.reply({ content: 'A categoria de tickets não foi definida.', ephemeral: true });
			}
			if (embedData.title === null || embedData.description === null) {
				return interaction.reply({ content: 'A embed de tickets não foi definida.', ephemeral: true });
			}
			if (!status) {
				await interaction.client.db.guild.setTickets(interaction.guild.id, true);
			}
			...
			await interaction.reply({ content: 'A mensagem foi enviada!', ephemeral: true });
		}
		if (interaction.options.getSubcommand() === 'suporte') {
			...
			if (supportRole.includes(role.id)) {
				...
				return interaction.reply({ content: `O cargo de suporte foi removido <@&${role.id}>.`, ephemeral: true });
			}
			else {
				await interaction.client.db.guild.setTicketsSupportRole(interaction.guild.id, role.id);
				return interaction.reply({ content: `O cargo de suporte foi adicionado <@&${role.id}>.`, ephemeral: true });
			}
		}
	}
};